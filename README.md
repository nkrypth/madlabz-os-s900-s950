...
- **Can you add another feature?** — I wish but the space left is limited. This sentence is already more than the space I have left.
- **Why the stock features were removed?** — MADLABZ OS has to fit in exactly the same space as the stock firmware, and on these machines the operating system shares one address space with sample memory. Keeping all of your sample RAM free (a hard rule here) means the OS can't grow — so every new feature is paid for in bytes freed by dropping a rarely-used stock one (the hex monitor, RS232, drum triggers, compressed saving, the ME35T editor).
- **Why S900 has more features than S950?**  — Stock OS for S950 already has much more densely packed code (SCSI, S1000 disk) so the space was limited.
- **Why S950 doesn't boot from OS disk?** — On a stock S950 the boot ROM can't cold-load MADLABZ straight from the floppy: the S950's boot path doesn't complete the disk load cleanly, so the OS won't come up on its own. Load it by hand instead — boot the stock OS, then use *Clear mem and load disk* (see *Running on Hardware* above). Booting straight from disk on power-on would require reflashing the S950's boot EPROMs, which isn't part of this floppy release.
